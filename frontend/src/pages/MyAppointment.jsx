...
const MyAppointment = () => {
  ...
  const getUserAppointments = React.useCallback(async () => {
    try {
    ...
    { appointments.map((item,index)=>(
      <div className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b' key={index}>
        ...
        <div className='flex-1 text-sm text-zinc-600'>
          ...
          <p className="text-sm mt-1"><span className="font-medium"> Date & Time :</span> {slotDateFormat(item.slotDate)} | {item.slotTime}</p>
        </div>
        <div>
